<template>
    <v-app class="futuristic-app">
        <v-card v-cloak id="droparea" ref="droparea" class="main-container"
            @drop.prevent="imgstore.doupload($event.dataTransfer?.files)" @dragover.prevent>
            <Snack ref="snack" />
            <v-layout>
                <!-- Futuristic App Bar -->
                <v-app-bar class="appbar-futuristic" prominent>
                    <v-app-bar-nav-icon v-if="is_mobile" variant="text" class="nav-icon-glow"
                    ...
                    <v-menu auto>
                            ...
                            <v-tooltip location="bottom">
                                <template v-slot:activator="tt">
                                    <span v-bind="tt.props">
                                        <v-btn text v-bind="menu.props" class="settings-btn">
                                            <v-icon>
                        ...
                        <v-list class="settings-menu">
                            <v-list-item>
                                <v-btn @click="toggleTheme" class="btn-neon-outline" block>
                            ...
                            <v-list-item>
                                <v-btn @click="goToLanding" class="btn-neon-outline" block>
                                    <v-icon size="20" class="mr-2">
                                        mdi-home
                                    </v-icon>
                                    Back to Home
                ...
                <v-navigation-drawer v-model="display_drawer" class="nav-futuristic" bottom>
                    ...
                    <v-list class="nav-list">
                        ...
                        <v-list-item @click="$router.push({ name: 'uploads', params: { idx: '0' } })"
                            :active="isactive('uploads')" class="nav-item">
                            <v-list-item-title class="d-flex align-center">
                        ...
                        <v-list-item @click="$router.push({ name: 'summary', params: { tab: 'detections' } })"
                            :active="isactive('summary')" class="nav-item">
                            <v-list-item-title class="d-flex align-center">
                ...
                <v-main class="main-content">
                    <v-container class="pa-6 pa-md-10">
                        ...
                        <div class="progress-container mb-7" v-if="imgstore.is_processing || imgstore.zip_busy">
                            <Progress :progress="imgstore.upload" v-if="imgstore.is_processing"
                                class="progress-neon mb-2" />
                            <Progress :progress="imgstore.process" v-if="imgstore.is_processing"
                                class="progress-neon mb-2" />
                            <Progress :progress="imgstore.zip" v-if="imgstore.zip_busy" class="progress-neon" />
                        </div>
